...
## English footnotes with Chinese glosses

English `chapterN.md` footnotes may add **Simplified Chinese only for the annotated head term** (title, person, place, set phrase). Do not sprinkle Chinese on every name in the explanation. See [Footnotes → Chinese head terms](markdown-syntax-extensions.md#chinese-head-terms-in-english-footnotes) in *Markdown syntax extensions*. English PDF rendering of those characters relies on the LuaLaTeX CJK font fallback (Noto Serif CJK SC).
